...
impor              {/* More Menu Dropdown - Positioned dynamically */}
              {showMoreMenu && (
                <div className={`absolute z-50 bg-white/20 backdrop-blur-xl border border-white/30 rounded-lg shadow-lg py-1 dropdown-menu ${
                  isCollapsed 
...
export function DesktopSidebar() {
  ...
  return (
    <>
      ...
      <div className={`fixed left-0 top-0 h-full transition-all duration-300 bg-gradient-to-b from-blue-600 to-teal-500 border-r border-white/30 z-40 hidden lg:block ${isCollapsed ? 'w-16 sidebar-collapsed' : 'w-64'}`}>
        <div className={`transition-all duration-300 ${isCollapsed ? 'p-2' : 'p-6'}`}>
          ...
          <nav className="space-y-2">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`relative flex items-center ${isCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-3'} rounded-lg text-sm font-medium transition-colors ${
                  pathname === item.href
              ...
              >
                ...
                {isCollapsed && item.name === "Chat" && totalUnreadCount > 0 && (
            ...
            {session && (
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  className={`w-full flex items-center ${isCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-3'} rounded-lg text-sm font-medium text-white/90 hover:bg-white/20 hover:text-white transition-colors`}
                  title={isCollapsed ? session.user?.name || "User Profile" : undefined}
                >
                  <Avatar className={`${isCollapsed ? 'h-6 w-6' : 'h-6 w-6 mr-3'}`}>
                ...
                {/* User Dropdown Menu - Positioned dynamically */}
                {showSignOutConfirm && (
                  <div className={`absolute z-50 bg-white/20 backdrop-blur-xl border border-white/30 rounded-lg shadow-lg py-1 dropdown-menu ${
                    isCollapsed 
                      ? 'left-16 bottom-0 min-w-48' 
            ...
            <div className="relative" ref={moreMenuRef}>
              <button
                ...
                className={`w-full flex items-center ${isCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-3'} rounded-lg text-sm font-medium transition-colors text-left ${
              ...
              {/* More Menu Dropdown - Positioned dynamically */}
              {showMoreMenu && (
                <div className={`absolute z-50 bg-white/20 backdrop-blur-xl border border-white/30 rounded-lg shadow-lg py-1 dropdown-menu ${
                  isCollapsed 
                    ? 'left-16 top-0 min-w-48' 
                    : 'bottom-full left-0 right-0 mb-2'
                }`}>
